...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_devices_device_name'), 'devices', ['device_name'])
    ...
    op.create_table(
        ...
        sa.Column('ph', sa.DECIMAL(precision=4, scale=2), nullable=True),
        sa.Column('ammonia', sa.DECIMAL(precision=6, scale=3), nullable=True),
        sa.Column('nitrite', sa.DECIMAL(precision=6, scale=3), nullable=True),
        sa.Column('oxygen', sa.DECIMAL(precision=5, scale=2), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('is_enabled', sa.SmallInteger(), server_default='1'),
        sa.Column('created_at', sa.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.Column('updated_at', sa.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP')),
        sa.ForeignKeyConstraint(['device_id'], ['devices.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    ...
    op.create_table(
        ...
        sa.Column('alarm_level', sa.Enum('提醒', '警告', '危险', name='alarmlevel'), nullable=False),
        ...
        sa.Column('is_resolved', sa.SmallInteger(), server_default='0'),
    ...
    op.create_table(
        ...
        sa.Column('mqtt_topic', sa.String(200), nullable=True),
        sa.Column('current_power', sa.DECIMAL(precision=5, scale=2), nullable=True),
        sa.Column('created_at', sa.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.Column('updated_at', sa.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP')),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.ForeignKeyConstraint(['device_id'], ['control_devices.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_table(
        ...
        sa.Column('spawn_date', sa.TIMESTAMP(), nullable=True),
        sa.Column('hatch_date', sa.TIMESTAMP(), nullable=True),
        ...
        sa.Column('feed_amount', sa.DECIMAL(precision=10, scale=2), nullable=True),
    ...
    op.create_index(op.f('ix_production_records_spawn_date'), 'production_records', ['spawn_date'])
    ...
    op.create_table(
        ...
        sa.Column('is_completed', sa.SmallInteger(), server_default='0'),
    ...
    op.create_table(
        ...
        sa.Column('backup_time', sa.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.Column('is_deleted', sa.SmallInteger(), server_default='0'),
        sa.Column('remark', sa.String(200), nullable=True),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_backups_id'), 'backups', ['id'])
    op.create_index(op.f('ix_backups_backup_type'), 'backups', ['backup_type'])
    ...
    # 创建ENUM类型
    mysql_type = mysql.VARCHAR if sa.__version__.startswith('1.4') else sa.String
    ...
    """)
    op.execute("INSERT INTO alarmlevel (name) VALUES ('提醒')")
    op.execute("INSERT INTO alarmlevel (name) VALUES ('警告')")
    op.execute("INSERT INTO alarmlevel (name) VALUES ('危险')")
...
def downgrade() -> None:
    ...
    # 删除ENUM类型
    op.execute("DROP TABLE IF EXISTS alarmlevel")
